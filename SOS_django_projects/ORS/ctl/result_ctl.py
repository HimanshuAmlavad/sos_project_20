from django.shortcuts import render
from ORS.ctl.BaseCtl import BaseCtl
from ORS.utility.HtmlUtility import HtmlUtility
from service.models import Result
from service.service.ResultService import ResultService
from service.utility.DataValidator import DataValidator


class ResultCtl(BaseCtl):

    def preload(self, request):
        grade = ["A+", "A", "B+", "B", "C", "D", "F"]

        status = ["Pass", "Fail", "ATKT", "Absent"]

        # print("Preload status:", repr(self.form.get("status")))

        self.preload_data["grade_select"] = HtmlUtility.get_list_from_list(
            "grade",
            self.form.get("grade"),
            grade,
        )
        self.preload_data["status_select"] = HtmlUtility.get_list_from_list(
            "status",
            self.form.get("status"),
            status,
        )
        # Also make preload available under form for templates using `form.preload_data`
        self.form["preload_data"] = self.preload_data
        return self.preload_data

    # Populate Form from HTTP Request
    def request_to_form(self, request):
        self.form["id"] = int(request.get("id", 0) or 0)
        # print('R2F =====================>', self.form["id"])
        self.form["result_id"] = request.get("resultId", 0)
        self.form["student_id"] = request.get("studentId", "")
        self.form["percentage"] = request.get("percentage", "")
        # print('R2F =====================>', self.form["percentage"])
        self.form["grade"] = request.get("grade", "")
        self.form["status"] = request.get("status", "")

    # Populate Form from Model
    def model_to_form(self, obj):
        if obj == None:
            return
        self.form["id"] = obj.id
        # print('M2F======================>', self.form["id"])
        self.form["result_id"] = obj.result_id
        self.form["student_id"] = obj.student_id
        self.form["percentage"] = obj.percentage
        print('M2F======================>', self.form["percentage"])
        self.form["grade"] = obj.grade
        self.form["status"] = obj.status
        # print('M2F======================>', self.form["status"])

    # Convert form into module
    def form_to_model(self, obj):
        pk = int(self.form.get("id", 0))
        if pk > 0:
            obj.id = pk
        # print('F2M======================>', obj.id)
        obj.result_id = int(self.form.get("result_id", 0))
        obj.student_id = self.form.get("student_id", "")
        obj.percentage = self.form.get("percentage", "")
        # print('F2M======================>', obj.percentage)
        obj.grade = self.form.get("grade", "")
        obj.status = self.form.get("status", "")
        return obj

    # Validate form
    def input_validation(self):
        super().input_validation()
        inputError = self.form["inputError"]
        if DataValidator.isNull(self.form["result_id"]):
            inputError["result_id"] = "Result Id is required"
            self.form["error"] = True
        if DataValidator.isNull(self.form["student_id"]):
            inputError["student_id"] = "Student Id is required"
            self.form["error"] = True
        if DataValidator.isNull(self.form["percentage"]):
            inputError["percentage"] = "Percentage is required"
            self.form["error"] = True
        if DataValidator.isNull(self.form["grade"]):
            inputError["grade"] = "Grade is required"
            self.form["error"] = True
        if DataValidator.isNull(self.form["status"]):
            inputError["status"] = "Status is required"
            self.form["error"] = True
        return self.form["error"]

    # Display Role page
    def display(self, request, params={}):
        if params["id"] > 0:
            fee = self.get_service().get(params["id"])
            self.model_to_form(fee)
        return render(
            request,
            self.get_template(),
            {"form": self.form, "preload_data": self.preload(request)},
        )

    # Submit Role page
    def submit(self, request, _params={}):
        result = self.form_to_model(Result())
        self.get_service().save(result)
        if int(self.form["id"]) > 0:
            self.form["id"] = result.id
        self.form["error"] = False
        self.form["message"] = "Data is saved"
        return render(
            request,
            self.get_template(),
            {"form": self.form, "preload_data": self.preload(request)},
        )

    # Template html of Role page
    def get_template(self):
        return "ors/result.html"

    # Service of Role
    def get_service(self):
        return ResultService()
